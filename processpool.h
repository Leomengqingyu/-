    ...
    : m_listenfd( listenfd ), m_process_number( process_number ), m_idx( -1 ), m_stop( false )
{
    ...
    m_sub_process = new process[ process_number ];//process_number 是逻辑服务器的个数这里是2.
                    ...
                    for( int i = 0; i < ret; ++i )
                    {
                        switch( signals[i] )
                        {
                            case SIGCHLD:
                            {
                                pid_t pid;
                                int stat;
                                while ( ( pid = waitpid( -1, &stat, WNOHANG ) ) > 0 ) //等收集退出的子进程，由于设置了WNOHANG因此不等待//不堵塞模式。避免堵塞导致其他传统信号读不到
                                {                                                     //因为管道事件注册了，子进程同样可以发现这个问题，不需要处理，只需要等待父进程处理就可以了
                                    continue;
                                }
                                break;
                            }
