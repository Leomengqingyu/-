...
RET_CODE conn::read_clt()
{
    ...
    while( true )
    {
        ...
        {
            log( LOG_ERR, __FILE__, __LINE__, "%s", "the client read buffer is full, let server write" );//客户端已经满了，此时本服务器读取数据转发给逻辑服务器
            return BUFFER_FULL;
        }

        bytes_read = recv( m_cltfd, m_clt_buf + m_clt_read_idx, BUF_SIZE - m_clt_read_idx, 0 );//从客户端中读取数据，第二个参数是指的是读取的位置，第三个参数指的是客户端剩余的空间。
        if ( bytes_read == -1 )
        {
...
RET_CODE conn::write_srv()
{
    ...
    while( true )
    {
        if( m_clt_read_idx <= m_clt_write_idx )//如果要写的数据多于读取的数据，缓冲区已经空了
        {
            m_clt_read_idx = 0;
            m_clt_write_idx = 0;
            return BUFFER_EMPTY;
        ...
        bytes_write = send( m_srvfd, m_clt_buf + m_clt_write_idx, m_clt_read_idx - m_clt_write_idx, 0 );//本机发送数据给服务器
        if ( bytes_write == -1 )
        {
            if( errno == EAGAIN || errno == EWOULDBLOCK )
            {
